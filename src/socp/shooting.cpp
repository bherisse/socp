...
struct shooting::data_struct{
	...
	std::vector< std::vector<int> > mode_X;	///< 0 for fixed X, 1 for free X, 2 for dependent time
	std::vector<int> mode_t;				///< 0 for fixed time, 1 for free time, 2 for continuity state
...
void shooting::SetMode(int const& mode_tf, std::vector<int> const& mode_Xf) const{
	data->mode_t[0] = 0;
	data->mode_X[0] = std::vector<int>(data->dim,0);
	data->mode_X[data->numMulti] = mode_Xf;
	for (int i=1; i<data->numMulti; i++){
		data->mode_X[i] = std::vector<int>(data->dim,2);
		data->mode_t[i] = 2;
	}
...
void shooting::SetMode(std::vector<int> const& mode_t, std::vector< std::vector<int> > const& mode_X) const{
	...
	data->mode_t = mode_t;
	// sum of free times to determine size of the problem to be solved.
	int sum_mode_t = 0;
...
*/
void shooting::SetContinuationMinStep(real const& step){
...
/**
* Compute Shooting with continuation step continuationStep
*/
...
/**
* Compute Shooting for any other real data Rdata with continuation step continuationStep
*/
...
void shooting::GetSolution(std::vector<real>& vt, std::vector<model::mstate>& vX) const{
	// update solution from computed solution (from data->tab_param to data->time and data->X)
	UpdateSolution();
	vt = data->time;
	for(int i=0;i<data->numMulti+1;i++){
		vX[i] = data->X[i];
...
void shooting::ShootingFunctionPar(int n, std::vector<real> const& param, std::vector<real> & fvec, int iflag) const
{
	...
	for (int i=0; i<data->numThread; i++){
		...
		data->mulThread[i].create(StaticShootingPar,(void*) data->userThreadData[i].data());
	}
...
int shooting::StaticShootingFunction(void *userData, int n, const real *param, real *fvec, int iflag){		// for cminpack
	...
	if (This->data->numThread <= 1){
		...
		This->ShootingFunctionPar(n, param_vec, fvec_vec, iflag);
	}
...
*/
void shooting::StaticShootingPar(void *arg){
	void **userArgs = (void**) arg;
	shooting* This = (shooting*) userArgs[0];
	...
	This->ShootingFunctionParThread(*n, param, fvec, *threadNum, *timeLine);
...
*/
void shooting::ShootingFunctionParThread(int n, real *param, real *fvec, int threadNum, std::vector<real> const& timeLine) const{
	// Data
